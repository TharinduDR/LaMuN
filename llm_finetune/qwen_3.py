"""
QLoRA instruction fine-tuning for Qwen/Qwen3-VL-8B-Instruct on LaMuN,
followed by automatic evaluation on the test set with BLEU-4, chrF, and CIDEr.

Mirrors the Qwen2.5-VL fine-tuning recipe (same prompt, same hyperparameters,
same loss masking, same QLoRA + LoRA configuration) so adapter rows are
directly comparable across Qwen generations.

Usage:
  python finetune_qwen3_vl_qlora.py                                # eng, 1000 train, full test
  python finetune_qwen3_vl_qlora.py --language sin --num-samples 1000
  python finetune_qwen3_vl_qlora.py --language ben --epochs 3 --eval-samples 200
  python finetune_qwen3_vl_qlora.py --skip-train --adapter-path ./.../final-adapter
  python finetune_qwen3_vl_qlora.py --skip-eval         # train only

Hardware:
  Qwen3-VL-8B in 4-bit + LoRA fits on a 24 GB A5000 with batch_size=1 and
  gradient accumulation. Allocate ~48 GB system RAM for safety during load.
...
class Qwen3VLDataCollator:
    ...
    Qwen3-VL's processor handles images directly via apply_chat_template when
    tokenize=True is passed, so we don't need qwen_vl_utils here.
    """

    def __init__(self, processor, language_code: str):
        self.processor = processor
        self.language_code = language_code
        # Same assistant marker as Qwen2.5-VL; both use the Qwen chat format.
    ...
    def __call__(self, examples):
        ...
        for ex in examples:
            ...
            # Build a fully-tokenised input including the assistant turn.
        ...
        # Pad and stack manually (per-example since image counts may vary)
        # NB: this assumes one image per example, which is true for LaMuN.
        input_ids = [b["input_ids"][0] for b in batch_inputs]
        attention = [b["attention_mask"][0] for b in batch_inputs]

        # Right-pad to the longest sequence
        pad_id = self.processor.tokenizer.pad_token_id
        max_len = max(x.size(0) for x in input_ids)
        padded_ids = torch.full(
            (len(input_ids), max_len), pad_id, dtype=input_ids[0].dtype
        )
        padded_mask = torch.zeros(
            (len(input_ids), max_len), dtype=attention[0].dtype
        )
        for i, (ids, mask) in enumerate(zip(input_ids, attention)):
            padded_ids[i, : ids.size(0)] = ids
            padded_mask[i, : mask.size(0)] = mask

        out = {"input_ids": padded_ids, "attention_mask": padded_mask}

        # Stack image-related tensors. Keys vary by transformers version; we
        # forward whatever the processor produced (pixel_values, image_grid_thw, etc.)
        for key in batch_inputs[0]:
            if key in ("input_ids", "attention_mask"):
                continue
            tensors = [b[key] for b in batch_inputs if key in b]
            if not tensors:
                continue
            try:
                out[key] = torch.cat(tensors, dim=0)
            except Exception:
                # Fall back to first example for non-stackable tensors
                out[key] = tensors[0]

        # Loss labels: mask pad + prompt portion
...
def generate_caption(model, processor, image, news_content, language_code,
    ...
    messages = [{
        ...
        "content": [
            ...
            {"type": "text", "text": build_prompt(news_content, language_code)},
        ],
    }]
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = f"./qwen3-vl-qlora-{args.language}-{args.num_samples}"
    ...
    if not args.skip_train:
        ...
        collator = Qwen3VLDataCollator(processor, args.language)
